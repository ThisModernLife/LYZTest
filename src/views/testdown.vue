<template>
    <div class="test">
        ...
        <div class="table-box" v-for="(item,index) in serveData"
            ...
            <p :class="index !== serveData.length-1 ? getResult[index+1][0].val === item.name ? 'active' : '' : ''">
                {{item.name}}
            </p>

            <p :class="index !== serveData.length-1 ? getResult[index+1][1].val === item.age ? 'active' : '' : ''">
                {{item.age}}
            </p>

            <p :class="index !== serveData.length-1 ? getResult[index+1][2].val === item.from ? 'active' : '' : ''">
                {{item.from}}
            </p>

            <p :class="index !== serveData.length-1 ? getResult[index+1][3].val === item.worktime ? 'active' : '' : ''">
                {{item.worktime}}
            </p>
...
<script>
    export default {
        ...
        mounted() {
            let that = this;
            console.log(that.getResult,that.serveData)
        },
        methods: {
            /**
             * 排除第一个的，所有内容
